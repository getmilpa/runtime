# Contributing to Milpa Runtime

Thanks for your interest in contributing! Milpa Runtime is the bootable Milpa kernel — it
composes `milpa/core`, `milpa/container`, `milpa/events`, `milpa/http`, and `milpa/plugin` into
a running app: a config-driven plugin registry, capability checks before boot, ordered plugin
boot with lifecycle events, and route-table assembly. Zero database, zero magic.
...
- **Respect the tier boundary.** Milpa Runtime composes the family (core/container/events/
  http/plugin), sits above them; never the reverse. Do not introduce a dependency on
  Doctrine, a concrete container, or any product/plugin code, and do not push
  kernel-composition concerns down into any of the packages it composes.
...
The whole Milpa family (`milpa/core`, `milpa/http`, `milpa/tool-runtime`, `milpa/runtime`)
shares one coding standard, committed verbatim in every repo as `.php-cs-fixer.dist.php` and
